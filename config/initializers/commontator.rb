...
Commontator.configure do |config|
  ...
  config.user_name_proc = lambda { |user| user.person.name }
  ...
  config.user_link_proc = lambda { |user, app_routes| app_routes.polymorphic_url(user.person) }
  ...
  config.user_avatar_proc = lambda { |user, view|
                                     view.image_tag(user.profile_image, size: '52x52', class: 'avatar') }
  ...
  config.thread_read_proc = lambda { |thread, user| user && user.payed_subscriber? }
  ...
  config.email_from_proc = lambda { |thread| "info@ipdb.net" }
  ...
  config.user_mentions_proc = lambda { |current_user, query|
    current_user.person.class.where('name LIKE ?', "#{query}%") }
end
